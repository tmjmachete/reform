...
import { createLesson, deleteCourse } from '@/app/school/admin/actions';
...
export default async function EditCoursePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="admin-page admin-narrow">
      <div className="admin-crumb"><Link href="/school/admin">Courses</Link> / {course.title}</div>
      <h1>Edit course</h1>
        ...
        <table className="admin-table">
          ...
          <tbody>
            {lessons.map((l, i) => (
              <tr key={l.id}>
                <td>{i + 1}</td>
                <td><Link href={`/school/admin/courses/${id}/lessons/${l.id}`} className="admin-link">{l.title}</Link></td>
